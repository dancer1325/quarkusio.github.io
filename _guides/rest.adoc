...
* goal
    ** how to write REST Services -- with -- Quarkus REST

== What is Quarkus REST?

* Quarkus REST
    ** == 👀NEW link:{jaxrsspec}[Jakarta REST (formerly JAX-RS)] implementation👀
        *** ⚠️FROM the scratch⚠️
        *** ⚠️fully reactive⚠️
            **** Reason:🧠based on Quarkus common https://vertx.io/[Vert.x] layer🧠
        *** tightly integrated -- with -- Quarkus
            **** == | build time, lot of work
        *** vs other Jakarta REST implementation
            **** performance | blocking & non-blocking endpoints
            ***** NEW features
...
* add `io.quarkus:quarkus-rest` extension
    ** if you create the Quarkus application via Quarkus Platform BOM -> 👀ALREADY included👀
    ** OTHERWISE,
        *** | Maven, add
+
...
* Endpoint
    ** method / called -- to -- serve a REST call
* URL / URI (Uniform Resource Locator / Identifier)
    ** == REST resources location
    ** https://tools.ietf.org/html/rfc7230#section-2.7[specification]
* Resource
    ** == your domain object
    ** == what your API serves & modifies
    ** | Jakarta REST, `entity`
* Representation of your resource | wire (internet)
    ** depend -- on -- content types (JSON, XML, HTML, ...)
* Content type OR media type
    ** == particular representation
        *** _Example:_ `text/plain` or `application/json`
* HTTP
    ** wire protocol -- for -- routing REST calls
    ** https://tools.ietf.org/html/rfc7230[HTTP specifications]
* HTTP request
    ** == HTTP method + target URI + headers + message body (OPTIONAL)
* HTTP response
    ** == HTTP response status + headers + message body (OPTIONAL)

=== Declaring endpoints: URI mapping

* link:{jaxrsapi}/jakarta/ws/rs/Path.html[`@Path`] annotation
    ** | class
        *** 👀's methods can be exposed -- as -- REST endpoints👀
            **** requirements
                ***** HTTP method annotation
        *** == URI prefix | its exposed methods
            **** OPTIONAL === can be `""`
    ** | exposed endpoint method
        *** == URI

* `@ApplicationPath` annotation OR `quarkus.rest.path` build time property
    ** 💡set ALL rest endpoints' root path💡
