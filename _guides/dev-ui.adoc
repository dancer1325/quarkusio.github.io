...
* goal
    ** Dev UI v2
    ** how to make your extension / visible | Dev UI

* Dev UI
    ** ships with Quarkus
    ** 👀AVAILABLE | dev mode (== `mvn quarkus:dev`) | http://localhost:8080/q/dev-ui[/q/dev-ui] 👀
+
image::dev-ui-overview-v2.png[]
    ** allows you
        *** view
            **** ALL extensions (status & documentation) CURRENTLY loaded
            **** link:dev-services.adoc[Dev Services information]
            **** Build information
        *** change Configuration`
        *** manage link:continuous-testing.adoc[continuous testing]
        *** stream various logs
* Dev UI v2
    ** == default one | Quarkus v3+

== Make my extension / extend the Dev UI

* ALL extensionS
    ** by default, are listed | Dev UI

* ways / extensions | Dev Ui, can
    - <<add-links-to-an-extension-card,Add links | extension card>>
...
To make build time data available on your full page, you can add any data to your `CardPageBuildItem` with a key and a value:
...
To build an interactive page with actions and runtime (or build time) data, you must use the web component page:
