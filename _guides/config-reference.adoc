...
* 👀Quarkus itself (core & extensions) + Quarkus application
** are configured -- via -- https://github.com/smallrye/smallrye-config[SmallRye Config] API 👀
* if you want
 ** MORE information -> <<additional-information,Additional Information>>
 ** to make a Quarkus extension configurable -> see
...
[source,properties]
----
# user-defined's configuration property
greeting.message=hello

# `quarkus-vertx-http` extension's configuration proprty
...
* uses
    ** configure DIFFERENTLY the application / target _environment_

* allows
    ** MULTIPLE configurations | same file OR separate files
    ** select BETWEEN configurations -- via a -- profile name
...
* syntax |
    ** "*.properties" -- `%{profile-name}.propertyName` --
...
** `.env` file -- `_{PROFILE}_CONFIG_KEY=value` --
...
* Quarkus features / -- inherited by -- link:https://github.com/smallrye/smallrye-config/[SmallRye Config]
