...
* goal
    ** how your Quarkus application can interact -- , via Quarkus Messaging -- with Apache Kafka
...
* 2 applications / communicate -- with -- Kafka
    ** first application
        *** sends a _quote request_ -- to -- Kafka
        *** consumes Kafka messages -- from the -- _quote_ topic
    ** second application
        *** receives the _quote request_
        *** sends a _quote_ back
...
* | dev mode OR tests
    ...
    ** 👀Quarkus starts AUTOMATICALLY -- via xref:kafka-dev-services.adoc[Dev Services for Kafka] -- a broker 👀

== Solution

* link:{quickstarts-tree-url}/kafka-quickstart[here]
...
:create-app-extensions: rest-jackson,messaging-kafka
:create-app-post-command:
include::{includes}/devtools/create-app.adoc[]

* used Quarkus extensions
    1. Quarkus REST (formerly RESTEasy Reactive) + its Jackson support
        *** serve the HTTP endpoint
    2. Reactive Messaging Kafka connector

=== _processor_ project

:create-app-artifact-id: kafka-quickstart-processor
:create-app-extensions: messaging-kafka
:create-app-post-command:
include::{includes}/devtools/create-app.adoc[]
