...
* goal
    ** how your web Quarkus application can provide -- , via https://opentelemetry.io/[OpenTelemetry] (OTel), -- distributed tracing
...
* ways to visualise the OpenTelemetry data
    ** xref:observability-devservices-lgtm.adoc[Grafana OTel LGTM system]
        *** -- for -- traces & metrics
    ** xref:#jaeger-to-see-traces-option[Jaeger]
        *** -- for -- traces
...
* https://github.com/open-telemetry/opentelemetry-specification/blob/main/specification/context/api-propagators.md[OpenTelemetry propagators]
    ** allows
        *** propagating cross-cutting concerns / share an underlying `Context`
    ** uses
        *** distributed (== cross MULTIPLE services) transaction

* OpenTelemetry extension
    ** 👀by default, enable the propagators👀
        *** https://www.w3.org/TR/trace-context/[W3C Trace Context]
        *** https://www.w3.org/TR/baggage/[W3C Baggage]
    ** if you want to customize which one to enable -> set <<configuration-reference, `propagators`>>

==== Additional Propagators

* TODO: The `b3`, `b3multi`, `jaeger` and `ottrace` propagators will need the https://github.com/open-telemetry/opentelemetry-java/tree/main/extensions/trace-propagators[trace-propagators]
