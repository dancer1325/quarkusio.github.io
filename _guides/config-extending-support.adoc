...
* `ConfigSource`
    ** requirements
        ...
        *** register the implementation -- via -- https://docs.oracle.com/en/java/javase/17/docs/api/java.base/java/util/ServiceLoader.html[ServiceLoader] |
    ...
    ** allows
        *** reading ADDITIONAL configuration values
        *** adding  ADDITIONAL configuration values | `Config` instance / ⚠️defined ordinal⚠️
...
|xref:config-reference.adoc#system-properties[System Properties]
|400

|xref:config-reference.adoc#environment-variables[Environment Variables from System]
|300

|xref:config-reference.adoc#env-file[Environment Variables from .env file]
|295
...
|===
+
image::/images/config-sources.png[align=center,width=90%]

=== Example
* TODO: create a project / use it
...
public class InMemoryConfigSource implements ConfigSource {
    ...
    public int getOrdinal() {
        return 275;
        // ordered between `.env` -- & -- `application.properties`
        // ==   if NOT find System Properties NOT Environment Variables from System NOT Environment Variables from .env file -> used
    }
...
* TODO: When a Quarkus application starts, a `ConfigSource` can be initialized twice. One time for _STATIC INIT_ and a second
