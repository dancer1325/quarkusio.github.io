...
* goal
    ** Quarkus application can interact -- , via Quarkus Messaging, with -- Apache Kafka

== Recommendations

* see PREVIOUSLY xref:kafka-getting-started.adoc[Getting Started to Quarkus Messaging -- with -- Apache Kafka]

== Introduction

* https://kafka.apache.org[Apache Kafka]
    ** TODO: is a popular open-source distributed event streaming platform.
...
* Messaging vocabulary | Kafka
    ** *message*
        *** == Kafka _record_
    ** *channel*
        *** Kafka connector maps _channels_ -- to -- Kafka _topics_
    ** **connector**
        *** `smallrye-kafka`
            **** specific Kafka connector

== Quarkus Extension for Apache Kafka

* link:messaging.adoc[SmallRye Reactive Messaging framework supported message backends]

* `messaging-kafka` extension
    ** == Quarkus  extension /
        *** 👀support -- , through https://smallrye.io/smallrye-reactive-messaging/[SmallRye Reactive Messaging framework], for -- Apache Kafka👀
        *** includes `kafka-clients` version 3.2.1 -- as a -- transitive dependency
        *** compatible -- with -- Kafka brokers version 2.x
...
* broker location
    ** ways
        *** globally -- `kafka.bootstrap.servers` OR `%profileName.kafka.bootstrap.servers` --
            **** xref:config-reference.adoc#profiles[/ profile]
        *** / channel -- `mp.messaging.incoming.$channel.bootstrap.servers` --
    ** <<kafka-dev-services>> AUTOMATICALLY starts a Kafka broker
        *** |
            **** dev mode
            **** run tests
...
* configure connector -- `mp.messaging.incoming.channelName.connector=smallrye-kafka` --
    ** 👀if you have 1! connector | your classpath -> you can omit the configuration👀
        *** Reason: 🧠Quarkus AUTOMATICALLY associates _orphan_ channels -- to the -- 1! connector🧠
...
* ways to create consumers
    ...
    ** `@Inject @Channel("KafkaTopicName")`
        *** application code
            **** is responsible -- for the -- subscription

* ⚠️requirements to consume ⚠️
    ** `mp.messaging.incoming.kafkaTopicName.connector`, OR
    ** `@Outgoing("kafkaTopicName")` | method

* ways to receive Kafka messages (valid for BOTH -- `@Incoming()` & `@Channel()`)
    ** receive message payload DIRECTLY
    ** access the incoming message metadata + handle MANUALLY the acknowledgment
        *** -- via -- `Message` type
        *** use <<commit-strategies, DIFFERENT acknowledgment strategies>>
    ...
    ** inject a `Multi` | your bean + `.subscribe()` to its events
        *** ALLOWED types
            **** `Multi<PrimitiveType>`
                ***** it acknowledges the message AUTOMATICALLY
                ***** support MULTIPLE subscribers
            **** `Multi<Message<PrimitiveType>>`
                ***** MANUALLY, you need to
                    ****** acknowledge
                    ****** <<broadcasting-messages-on-multiple-consumers, broadcast>>
            **** `Publisher<PrimitiveType>`
            **** `Publisher<Message<PrimitiveType>>`

* `Record`
    ** == wrapper around incoming Kafka record's key and payload

[[blocking-processing]]
=== Blocking processing

* Reactive Messaging
    ** by default, xref:quarkus-reactive-architecture.adoc[invoke your method | I/O thread]
        *** == ❌threads NOT blocked❌
    ** + blocking processing (`@Blocking`)
        *** use cases
            **** database interactions
        *** ❌should NOT run | caller thread❌
    ** Kafka connector
...
* `@Blocking` annotations
    ** EXISTING
        1. `io.smallrye.reactive.messaging.annotations.Blocking`
            *** MORE fine-grained tuning
                **** _Examples:_
                    ***** worker pool -- to -- use
                    ***** worker pool / preserves the order
        2. `io.smallrye.common.annotation.Blocking`
            *** uses the default Quarkus pool
            *** preserves the order processing of messages
            *** compatible -- with -- other Quarkus reactive features
    ** https://smallrye.io/smallrye-reactive-messaging/latest/concepts/blocking/[MORE information]

* `@RunOnVirtualThread`
    ** allows
        *** | Java _virtual threads_, running the blocking processing
    ** xref:messaging-virtual-threads.adoc[MORE]
...
* _Examples:_
    ** link:{quickstarts-tree-url}/kafka-panache-quickstart[here]

=== Acknowledgment Strategies

* ALL messages / received by a consumer
    ** ⚠️requirements⚠️
        *** acknowledge
            **** ❌OTHERWISE, the processing == error❌

* Strategies
    ** `Strategy.POST_PROCESSING`
        *** default one
        *** if the consumer method
            **** receives a `Record` OR payload -> the message will be acked | method return
                ***** ⚠️if the message fails -> Kafka retries -> NOT lose the message⚠️
            **** returns another reactive stream OR `CompletionStage` -> the message will be acked | ack the downstream message
                ***** ⚠️if the message fails -> Kafka retries -> NOT lose the message⚠️
    ** `Strategy.PRE_PROCESSING`
        *** ack the message | arrival
            **** == ⚠️if the message fails -> Kafka does NOT retry -> lose the message⚠️
    ** `Strategy.NONE`
        *** ❌NOT ack the message❌
            **** ⚠️if you want to ack -> you need to ack MANUALLY⚠️
    ** `Strategy.MANUAL`
        *** ⚠️you need to handle ack/nack the message⚠️

[[commit-strategies]]
=== Commit Strategies

* | ack a message / produced -- from a -- Kafka record,
    ** Kafka connector invokes a commit strategy

* Commit strategies
    ** responsible for
        *** commit the consumer offset / specific topic/partition is

* Commit an offset
    ** == ALL previous records have been processed
    ** position |
        *** after a crash recovery OR restart, application would restart the processing
    ** has performance penalties
        *** Reason:🧠Kafka offset management can be slow🧠
    ** ⚠️if the application crashes BETWEEN 2 commits & NOT commit the offset OFTENLY -> may lead to message duplication⚠️

* Kafka connector supported strategies
...
[IMPORTANT]
====
The Kafka connector disables the Kafka auto commit when it is not explicitly enabled
