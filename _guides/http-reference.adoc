...
* goal
 ** EXISTING HTTP functionalities | Quarkus

* Eclipse Vert
 ** fundamental HTTP layer
* on top layers
 ** Servlet support -- via -- customized Undertow version
 ** Jakarta REST support -- via -- RESTEasy
  *** if Undertow is used -> RESTEasy -- is used as a -- Servlet filter
  *** if Undertow is NOT used -> RESTEasy operates (directly) | Vert.x
   **** / NO use Servlets

== Serving static resources

* if you want to use Quarkus | web application -> see xref:web.adoc[Quarkus for the Web]

=== -- From -- application jar

* requirements
 ** place static resources | `META-INF/resources`
  *** Reason: 🧠standard location / resources in `jar` files -- defined by the -- Servlet spec🧠
* TODO: Even though
