...
* ArC
    ** == Quarkus' CDI container / bootstrapped | build time
        *** ways to integrate with the container
            **** https://jakarta.ee/specifications/cdi/4.1/jakarta-cdi-spec-4.1.html#spi_lite[CDI Build Compatible Extensions, window="_blank"]
            **** Quarkus-specific extension API

* CDI Portable Extensions
    ** ❌NOT supported❌

* goal
    ** Quarkus-specific extensions API
...
* container bootstrap phases
    ...
    2. Bean discovery
        *** container
            **** analyzes ALL application classes
            **** identifies beans
            **** wires ALL beans -- based on the -- provided metadata
    3. Extensions can register synthetic components
        *** synthetic components' attributes are FULLY controlled -- by the -- extensions (NOT -- from an -- EXISTING class)
    4. Container validates the deployment
        ***  == validates every application's injection point
        *** if there is NO bean / satisfies the given required type & qualifiers -> fails the build
...
* Classes & annotations
    ** == primary source of bean-level metadata

* metadata
    ** initial one read -- from the -- _bean archive index_
    ** | certain stages of the bootstrap,
        *** extensions can add, remove or transform the metadata

* bean archive index
    ** == https://github.com/smallrye/jandex[Jandex index, window="_blank"]
    ** immutable
    ** | <<cdi-reference.adoc#bean_discovery,bean discovery>>,
...
* _synthetic bean_
    ** 's bean attributes
        *** are NOT derived -- from a -- Java class, method or field
        *** are defined -- by an -- extension
        *** are registered
            **** | regular CDI, -- via --
                ***** https://jakarta.ee/specifications/cdi/4.1/jakarta-cdi-spec-4.1.html#after_bean_discovery[`AfterBeanDiscovery.addBean()`, window="_blank"]
                ***** https://jakarta.ee/specifications/cdi/4.1/jakarta-cdi-spec-4.1.html#bce_synthesis[`SyntheticComponents.addBean()`]
            **** -- via -- `SyntheticBeanBuildItem`
