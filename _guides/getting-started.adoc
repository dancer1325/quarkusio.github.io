...
* 1! resource instance
    ** ⚠️!= 1 resource instance / request⚠️
...
* https://maven.apache.org/surefire/maven-surefire-plugin/[Surefire Maven Plugin]
    ** requirements
        *** set the `<version>`
            **** Reason:🧠default version does NOT support JUnit 5🧠
+
...
----
        ...
        *** set `<java.util.logging>` system property
            **** Reason: 🧠make sure tests will use the correct log manager🧠
        *** set `<maven.home>`
            **** Reason:🧠ensure apply custom configuration -- from -- `${maven.home}/conf/settings.xml` 🧠
...
* Quarkus' build time
    ** 👀discover >1 classes OR annotations -- via -- https://github.com/smallrye/jandex[Jandex]👀
        *** _Example:_ CDI bean discovery
            **** xref:cdi-reference.adoc#bean_discovery[MORE]
        *** | Quarkus 2.0+,
            **** ❌NOT create separated Jandex index file❌

* | multi-module project,
    ** xref:maven-tooling.adoc#multi-module-maven[Maven]
    ** xref:gradle-tooling.adoc#multi-module-gradle[Gradle]
