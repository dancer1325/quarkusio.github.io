...
* microserviceS' challenge
   ** communication -- with -- external systems: ⚠️unreliable⚠️
      *** Reason:🧠microservice architecture == distributed🧠
      *** -> demand on: resiliency of applications

* https://github.com/smallrye/smallrye-fault-tolerance/[SmallRye Fault Tolerance]
   ** allows
      *** making MORE resilient applications
   ** == implementation of https://github.com/eclipse/microprofile-fault-tolerance/[MicroProfile Fault Tolerance specification]

* goal
   ** how to use MicroProfile Fault Tolerance annotations
      *** _Examples:_ `@Timeout`, `@Fallback`, `@Retry`, `@CircuitBreaker`, and `@RateLimit`
...
* application built | this guide
   ** == gourmet coffee e-shop's simple backend /
      *** some methods require communication -- to -- external services (database, external microservice, ...)
         **** == unreliability

== Solution

* link:{quickstarts-tree-url}/microprofile-fault-tolerance-quickstart[here]
