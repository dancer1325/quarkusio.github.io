...
* "http://localhost:8080/q/dev-ui" & _Services_ | gRPC tile
    ** your gRPC services | Quarkus Dev UI

* requirements
    ** application -- MUST export -- "normal" HTTP port
        *** Reason: 🧠Dev UI -- is -- accessible🧠
    ** if your application does NOT expose HTTP endpoints -> create a dedicated profile -- using --
+
[source,xml]
----
    <profiles>
