...
* Quarkus DI solution OR ArC
    ** -- based on the -- https://jakarta.ee/specifications/cdi/4.1/jakarta-cdi-spec-4.1.html[Jakarta Contexts and Dependency Injection 4.1 specification, window="_blank"] /
        *** implements the CDI Lite specification (passes the CDI Lite TCK) + improvements
        *** ❌NOT implement FULLY CDI❌
    ** work | MOST of the existing CDI code
...
* Bean discovery
    ** | CDI,
        *** == complex process / involves
            **** legacy deployment structures
            **** module architecture's accessibility requirements
    ** Quarkus uses a *simplified bean discovery*
        *** == 1! bean archive /
            **** https://jakarta.ee/specifications/cdi/4.1/jakarta-cdi-spec-4.1.html#default_bean_discovery[bean discovery mode `annotated`, window="_blank"]
...
* bean archive
    ** is synthesized
        ...
        *** from
            **** application classes,
            **** dependencies / contain a `beans.xml` descriptor (content is ignored)
            **** dependencies / contain a Jandex index - `META-INF/jandex.idx`
            **** dependencies / referenced by `quarkus.index-dependency` | `application.properties`
            **** Quarkus integration code

* Bean classes /
    ** NOT have a https://jakarta.ee/specifications/cdi/4.1/jakarta-cdi-spec-4.1.html#bean_defining_annotations[bean defining annotation, window="_blank"]
    ** NOT discovered
    ** defined -- by -- CDI
...
* if a dependency has a Jandex index -> AUTOMATICALLY scanned for beans

* ways to generate a Jandex index
    ** add a plugin
+
...
** add `quarkus.index-dependency` entries | your `application.properties`
+
[source,properties]
----
quarkus.index-dependency.<name>.group-id=

# if you do NOT specify artifactId -> index ALL dependencies / specific `group-id`
...
* Quarkus
    ** builds a native executable -- via -- GraalVM
    ** recommendations
        *** ❌NOT use private members | 's beans❌
            **** Reason:🧠Quarkus DI accesses -- , via reflection, to -- private member🧠
                ***** ==
                    ****** injection fields, constructors and initializers
                    ****** observer methods
                    ****** producer methods and fields
                    ****** disposers and interceptor methods
            **** ALTERNATIVES
...
public class CounterBean {
    ...
    @Inject
    CounterService counterService; // package-private injection field

    // package-private observer method
...
* GraalVM's limitations
    ** ⚠️usage of link:https://www.graalvm.org/{graalvm-docs-version}/reference-manual/native-image/Reflection/[Reflection, window="_blank"]⚠️
        *** == reflective operations
            **** are supported
            **** ALL relevant members must be registered EXPLICITLY for reflection
                ***** -> ⚠️bigger native executable⚠️
...
* | CDI,
    ** producer method ALWAYS needs to be annotated -- with -- `@Produces`

* | Quarkus,
    ** if the producer method is annotated with a scope annotation, a stereotype or a qualifier -> you can skip `@Produces`
