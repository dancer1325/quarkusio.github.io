...
* Quarkus' web security layer
    ** pluggable
    ** if it's enabled -> check permissions | ALL HTTP requests

* `@PermitAll`
    ** 👀's priority < `quarkus.http.auth.` 's priority👀

* if you use Jakarta RESTful Web Services & you want to set default security requirements -> 👀use `quarkus.security.jaxrs.deny-unannotated-endpoints` OR `quarkus.security.jaxrs.default-roles-allowed` 👀
    ** instead of `quarkus.http.auth.`
        *** Reason:🧠higher priority🧠

* Authorization
    ** 👀is -- based on -- user roles👀 /
        *** security provider provides
        *** if you want to customize these roles -> create a
xref:security-customization.adoc#security-identity-customization[`SecurityIdentityAugmentor`]
...
s| `deny` | denies ALL users
s| `permit` | permits ALL users
s| `authenticated` | permits ONLY authenticated users
|===

* role-based policies
    ** can be defined
    ** enable users / specific roles -- to -- access the resources
    ** _Example:_
+

[source,properties]
----
#  enable users / user & admin roles
...
[source,properties]
----
# permission / refers the default built-in `permit` policy
quarkus.http.auth.permission.permit1.paths=/public/*
quarkus.http.auth.permission.permit1.policy=permit
quarkus.http.auth.permission.permit1.methods=GET

# permission / refers the default built-in `deny` policy
quarkus.http.auth.permission.deny1.paths=/forbidden         # ALSO secures `/forbidden/`
quarkus.http.auth.permission.deny1.policy=deny

# permission / refers the PREVIOUS custom `role-policy1` policy
