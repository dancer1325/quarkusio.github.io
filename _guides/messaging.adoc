...
* Event-driven messaging systems
    ** enable building
        ...
        *** complex data streaming pipelines

* Quarkus
    ** provides
        *** suite of messaging extensions / synchronize with leading messaging technologies

image::messaging-quarkus.png[Quarkus Messaging]

* Quarkus extensions /
    ** maintained | core Quarkus repository
        *** *Messaging*
            **** == basic concepts + APIs
            **** uses
    ...
    ** maintained | community
        ...
        *** AWS SQS Connector
    ** low-level provider-specific integrations
...
* Messaging vocabulary
    ** *message*
        *** == _payload_ + _metadata_
            **** _metadata_
                ***** OPTIONAL
        *** applications send and receive them
    ** *channel*
        *** == via | messages transit on
        *** allows
            **** application components can publish and consume messages
        *** are connected -- , via connectors, to -- message backends
        *** _Orphans_ channel
            **** == outgoing channels WITHOUT a downstream consumer
            **** == incoming channels WITHOUT an upstream producer
    ** *connectors*
        *** are configured -- to --
            **** map incoming messages -- to a -- specific channel (consumed -- by the -- application)
            **** collect outgoing messages sent -- to a -- specific channel
        *** specific / EACH messaging technology

== Quarkus Messaging Development Model
* Quarkus Messaging Development Model
    ** 👀== uniform model👀
        *** -- for -- publishing + consuming + processing messages
        *** regardless the underlying broker technology (message queuing, event streaming, ...)
    ** 💡built | https://github.com/dancer1325/microprofile-reactive-messaging[MicroProfile Reactive Messaging specification]💡

=== `@Incoming` and `@Outgoing` annotations

* `@Incoming` method annotations
    ** == _channels_ / consume messages -- from the -- message broker
    ** use cases
        *** by itself | method, to consume messages
    ** uses
        *** by the framework
            ****❌NOT invoked DIRECTLY -- from the -- code
* `@Outgoing` method annotations
    ** == _channels_ / produce messages -- to the -- message broker
    ** use cases
        *** by itself | method, to generate messages
    ** uses
        *** by the framework
            ****❌NOT invoked DIRECTLY -- from the -- code❌

* link:{rm_doc_method_signatures}[SmallRye Reactive Messaging – Supported signatures]
...
* use case
    ** application / message + other parts of the application
        *** _Example:_ | HTTP endpoints, produce messages

* `@Channel`
    ** == channel |
        *** send your payloads OR messages
        *** inject the stream of messages
    ** | consume messages / sent -- via -- `@Channel`
        *** -> ⚠️application code is responsible for subscribing | stream⚠️
            **** == MANUAL subscription
                ***** _Examples:_ programmatically, via REST
            **** ❌!= `@Incoming` (== AUTOMATIC)❌

* `Emitter`
    ** allows
        *** buffering (== if there are NO consumers -> keep the messages) messages / sent to the channel
    ** vs
        *** link:{mutiny}[Mutiny APIs]
            **** Mutiny has MORE control
                ***** sync
                ***** async
            **** `MutinyEmitter`

* link:{rm_doc_emitter}[MORE]

=== Messages and Metadata

* `Message`
    ** == 💡wrap the payload💡
        *** `Message<T>` / payload of type `<T>`
            **** built-in `Metadata`
            **** `<T>` can be arbitrary objects
            **** built-in async actions -- for --
                ***** acknowledgement (`.ack()`) &
                ***** negative acknowledgement (`.nack()`)
    ** ⚠️exist / EACH payload⚠️

* TODO: A message is acknowledged back to the broker when its processing or reception has been successful.
...
In most cases, acks and nacks are managed for you and connectors allow you to configure
...
Connectors add specific metadata objects to the message to give access to the message headers, properties,
and other connector-specific information.
...
You can implement a custom `MessageConverter` to transform the payload to a type that
is accepted by your application.
