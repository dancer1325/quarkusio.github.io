...
* goal
    ** how your Quarkus application can -- , via https://opentelemetry.io/[OpenTelemetry] (OTel), -- provide
...
* OTHER guides
    ** xref:opentelemetry-tracing.adoc[OpenTelemetry Tracing Guide]
        *** ⚠️by default, *on*⚠️
    ** new xref:opentelemetry-metrics.adoc[OpenTelemetry Metrics Guide]
        *** ⚠️by default, *of*⚠️
            **** if you want to enable it -> `quarkus.otel.metrics.enabled=true`
    ** xref:opentelemetry-logging.adoc[OpenTelemetry Logging Guide]
        *** ⚠️by default, *of*⚠️
            **** if you want to enable it -> `quarkus.otel.logs.enabled=true`

* recommendations
    ** ❌NOT use https://opentelemetry.io/docs/instrumentation/java/automatic/[OpenTelemetry Agent]❌
        *** Reasons:🧠
            **** Quarkus Extensions & libraries / they provide, are directly instrumented
            **** the agent does NOT work with native mode🧠
...
* https://opentelemetry.io/docs/what-is-opentelemetry/[OpenTelemetry]
    ** == Observability framework & toolkit /
        ...
        *** 's goal
            **** create & manage telemetry data (traces, metrics, and logs)
    ** provides
        *** for tracing
            **** manual & automatic instrumentation
                ***** -> can be observable -- by -- tools & services / support OpenTelemetry
        *** for metrics
            **** manual instrumentation capabilities
                ***** -> can be observable -- by -- tools & services / support OpenTelemetry
    ** 👀vs xref:telemetry-micrometer.adoc[Quarkus Micrometer extension]👀
        *** allows
...
* Quarkus OpenTelemetry extension
    ** uses its OWN signal exporters | Vert.x
        *** Reason:🧠optimal performance and maintainability🧠
        *** 👀AUTOMATICALLY wired -- with -- CDI👀 that's why the `quarkus.otel.traces.exporter` and `quarkus.otel.metrics.exporter` properties default to `cdi`.
...
* `quarkus.otel.exporter.otlp.endpoint`
    ** is automatically set to the OTel collector endpoint as seen from the outside of the Docker container
    ** ⚠️-- depends on -- `quarkus.otel.exporter.otlp.protocol`⚠️
        *** if `quarkus.otel.exporter.otlp.protocol=grpc` -> `4317`
        *** if `quarkus.otel.exporter.otlp.protocol=http/protobuf` -> `4318`
