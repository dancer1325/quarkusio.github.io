...
* Quarkus
    ** AUTOMATICALLY generate Kubernetes resources
        *** -- ,via https://github.com/dekorateio/dekorate/[dekorate], based on --
            **** sane defaults
            **** user-supplied configuration
        *** support
            **** vanilla <<#kubernetes,Kubernetes>>,
            **** <<#openshift,OpenShift>>
            **** <<#knative,Knative>>
    ** can deploy the application | target Kubernetes cluster
        *** steps
            **** add xref:container-image.adoc[a container image extension]
                ***** Reason:🧠
                    ****** create a container image
                    ****** push the container image | registry🧠
            **** apply the generated manifests | target cluster's API Server
...
* Access to a Kubernetes cluster
    ** _Example:_ -- via -- Minikube

[[kubernetes]]
== Kubernetes

* goal
    ** create a project / contains Kubernetes extension + Jib extension
...
* | package,
    ** 👀`./mvnw package` -> generate Kubernetes manifests (`Deployment`, `Service`)👀
        *** | "target/kubernetes/",
            **** kubernetes.json
            **** kubernetes.yml
        *** `spec.template.spec.containers[*].image` == `yourDockerUsername/kubernetes-quickstart:1.0.0-SNAPSHOT`
...
* https://github.com/dancer1325/quarkus-quickstarts/tree/main/kubernetes-quickstart[here]

=== Generating idempotent resources

When generating the Kubernetes manifests, Quarkus automatically adds some labels and annotations to give extra information about the generation date or versions
For example:
...
You can configure the rest of the Kubernetes Job configuration using the properties under `quarkus.kubernetes.job.xxx` (see xref:deploying-to-kubernetes.adoc#quarkus-kubernetes_quarkus-kubernetes-job-parallelism[link]).
...
You can configure the rest of the Kubernetes CronJob configuration using the properties under `quarkus.kubernetes.cron-job.xxx` (see xref:deploying-to-kubernetes.adoc#quarkus-kubernetes_quarkus-kubernetes-cron-job-parallelism[link]).
...
* common changes | generated manifests
    ** `imagePullPolicy=IfNotPresent`
    ** `NodePort=Service`

* Quarkus extensions / set ADDITIONAL options by default
    ** quarkus-minikube
        *** use cases: Minikube, Docker Desktop, microk8s
    ** quarkus-kind
...
* `quarkus-minikube` extension
    ** generate Kubernetes manifests (`minikube.yaml` and `minikube.json`) / tailored to Minikube
    ** ways to use it
        *** | Maven
+
[source,xml,role="primary asciidoc-tabs-target-sync-cli asciidoc-tabs-target-sync-maven"]
...
* `quarkus-kind` extension
    ** generate Kubernetes manifests (`kind.yaml` and `kind.json`) / tailored to
        *** Kind
        *** | perform container image builds -> AUTOMATICALLY load images | cluster
    ** ways to use it
        *** | Maven
+
[source,xml]
...
== Tuning the generated resources -- via -- application.properties
...
==== Client Connection Configuration == connection -- to -- Kubernetes cluster
* by default,
    ** AUTOMATICALLY uses the `kubectl`'s active _context_
