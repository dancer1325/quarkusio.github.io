...
* Quarkus
    ...
    ** can deploy the application | target Kubernetes cluster
        *** -- via -- applying the generated manifests | target cluster's API Server
        *** PREVIOUSLY, if xref:container-image.adoc[a container image extensions is present] -> can
...
* | package,
    ** generate Kubernetes manifests (`Deployment`, `Service`)
        *** | "target/kubernetes/",
            **** kubernetes.json
            **** kubernetes.yml
