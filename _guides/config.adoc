...
* Quarkus application configuration
    ** 👀-- based on -- https://github.com/smallrye/smallrye-config[SmallRye Config API]👀
...
* Quarkus
    ** inject configuration properties | application -- via --
        *** 💡 https://microprofile.io/project/eclipse/microprofile-config[MicroProfile Config] annotations💡
            **** ways
                ***** `@ConfigProperty` OR
                ***** `@Inject @ConfigProperty`
                    ****** ❌`@Inject` NOT necessary❌
            **** ⚠️if the application tries to inject a configuration property / NOT set -> throws an error⚠️
...
* 👀if you change the configuration file -> IMMEDIATELY reflected WITHOUT shut down👀
