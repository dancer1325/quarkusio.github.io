...
* Quarkus' `create-extension`
    ** == Maven Mojo /
        ...
        *** try to auto-detect its options
            **** FROM `quarkus` (Quarkus Core) OR `quarkus/extensions` directory,
                ***** use the 'Quarkus Core' extension layout + defaults
            **** + `-DgroupId=io.quarkiverse.[extensionId]`,
                ***** use the 'Quarkiverse' extension layout + defaults
            **** REST of cases,
                ***** use the 'Standalone' extension layout + defaults
            **** | future,
                ***** other layout types can be added
...
* Gradle extension plugin
    ** ⚠️experimental⚠️
        *** may miss features / AVAILABLE | Maven plugin

* Quarkus
    ** NOT provide any way -- to -- initialize a Gradle project -- for -- extensions
