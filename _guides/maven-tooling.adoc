...
* Maven
    ** allows you
        ...
        *** build your application -- into -- jar / native executable / container-friendly executable
        *** import your project -- via Maven project metadata -- | your favorite IDE
...
* 👀ALLOWED attributes | `create` command👀
...
| `projectGroupId`
| `org.acme.sample`
| created project's groupId

| `projectArtifactId`
| _mandatory_
| created project's artifactId. if you do NOT pass -> interactive mode. == directory name. if it ALREADY exists -> generation fails

| `projectVersion`
| `1.0.0-SNAPSHOT`
| created project's version

| `platformGroupId`
| `{quarkus-platform-groupid}`
| target platform's groupId

| `platformArtifactId`
| `quarkus-bom`
| target platform BOM's artifactId

| `platformVersion`
| recommended by the xref:extension-registry-user.adoc[Quarkus Extension Registry]
| if you specify a range -> latest from the specified range
...
| `className`
| if omitted -> NOT created
| generated resource's FULLY qualified name

| `path`
| `/hello`
| resource path. ONLY relevant if `className` is set
...
| `true`
| use OR NOT the online registry -- to -- resolve extension catalogs. if false -> extension catalog == platform BOM's catalog

|===

* `{quarkus-platform-groupid}:quarkus-bom:{quarkus-version}`
    ** == default target

* SEVERAL Dockerfiles
    ** are generated | "src/main/docker"
    ** -- for --
        *** native mode
        *** jvm mode
...
:add-extension-extensions: hibernate-validator,quarkus-spring-cache
...
* enable
    ** ⭐️hot deployment + background compilation ⭐️
        *** == if you modify your ".java" OR resource files & refresh your browser -> these changes -- will AUTOMATICALLY -- deployed
        *** if there are issues with compilation OR deployment -> error page -- will -- let you know
    ** link:dev-ui.adoc[Dev UI]

* debug host
    ** by default, `localhost`
    ** if you need to change it -> specify `-DdebugHost`
        *** https://www.jetbrains.com/help/idea/tutorial-remote-debug.html[IntelliJ's remote debug]
        *** _Example:_ debugging | ALL hosts
...
* Quarkus
    ** by default,
        *** ❌does NOT discover CDI beans | ANOTHER module❌
