...
[[project-creation]]
=== Creating a new project -- `quarkus create` --
...
* ways
+
    1. specify xref:platform.adoc#quarkus-platform-bom[Platform release BOM version]
        *** `--platform-bom=groupId:artifactId:version` OR `-P :quarkus-bom:`
        *** 👀if you specify 1! segment (== NO `:`) -> it's the version👀
            **** `{quarkus-platform-groupid}` == default `groupId`
        *** `--dry-run` option
            **** see the computed value WITHOUT being calculated

    2. Specify a Platform Stream
        *** TODO: platform stream operates against a remote registry
        *** Each registry defines one or more platform streams, and
each stream defines one or more platform release BOM files that define how projects using that stream should be configured.
        *** Streams are identified using `platformKey:streamId` syntax
        *** A specific stream can be specified using `-S platformKey:streamId`
        *** When specifying a stream, empty segments will be replaced with _discovered_ defaults, based on stream resource resolution rules.
...
* `quarkus ext ls`
    ** ways to configure the output
        *** `--name`
            **** ONLY the name (artifactId)
        *** `--concise`
            **** ONLY name (artifactId) & description
        *** `--full`
            **** FULL information
        *** `--origins`
            **** Quarkus platform release origin & name & version

===== -- for a -- Quarkus release

* ==👀list Quarkus CLI release's ALL AVAILABLE extensions👀
    ** if you want to specify the Quarkus release -> use `-P` or `-S`
        *** <<specifying-quarkus-version, here>>
* | NO quarkus project,
    ** `quarkus ext ls`
...
* `quarkus ext add extension1 extension2 ...`
    ** `extensioni` can match a glob pattern

==== Removing extensions

* `quarkus ext remove extension1` OR `quarkus ext rm extension1`

=== Build your project

* `quarkus build`
    ** 's output depend -- on the -- your project's build tool (Maven, Gradle, or JBang)
    ** `quarkus build --clean`
