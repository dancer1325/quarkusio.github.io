...
* goal
    ** interact -- , via REST Client, with -- REST APIs

* REST Client
    ** == 👀REST Client implementation👀 /
        *** compatible -- with -- Quarkus REST (formerly RESTEasy Reactive)
    ** ❌!= Quarkus REST❌
...
* link:{quickstarts-tree-url}/rest-client-quickstart[here]
...
* `rest-jackson` extension
    ** -- for the -- REST server support
    ** ❌if you do NOT use Jackson -> skip it & use `rest`❌
* `rest-client-jackson` extension
    ** -- for the -- REST client support
    ** ❌if you do NOT use `rest-client` -> skip it & use `rest-jackson`❌
...
* `@RegisterRestClient`
    ** 💡informs Quarkus / this interface is available for CDI injection -- as a -- REST Client💡
...
* query parameter
    ** 👀ways to specify👀
        *** annotate a client method parameter -- with --
            **** `@QueryParam` OR
            **** `@RestQuery`
                ***** vs `@QueryParam`
                    ****** optional name
                    ****** query parameters
        *** annotate REST client interface OR interface's method -- with -- `@ClientQueryParam`
...
* `@ClientQueryParam`
    ** `.value`
        *** ALLOWED values
            ...
            **** determined -- by -- invoking a method
