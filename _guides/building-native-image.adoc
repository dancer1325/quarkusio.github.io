...
* goal
    ...
    ** Packaging the native executable | container
...
* application / developed | xref:getting-started.adoc[Done Getting Started Guide]
* xref:configuring-c-development[working C development environment]

.Supporting native compilation | C
[[configuring-c-development]]
[NOTE]
====
* | Linux, you need GCC & glibc & zlib headers
    ** _Examples:_ | COMMON distributions
+
[source,bash]
----
# dnf (rpm-based)
...
----
* | macOS, you need XCode CL
+
[source,bash]
----
xcode-select --install
----
* | Windows, install the https://aka.ms/vs/15/release/vs_buildtools.exe[Visual Studio 2017 Visual C++ Build Tools]
====

=== Background

* requirements to build a NATIVE executable
    ** distribution of GraalVM / EXIST
        *** Oracle GraalVM Community Edition (CE),
        *** Oracle GraalVM Enterprise Edition (EE)
        *** Mandrel
            **** == Oracle GraalVM CE's downstream distribution /
                ***** exclude UNNECESSARY -- for -- Quarkus native apps
                    ****** -> REDUCED 's size
                ***** ❌NOT support for polyglot programming ❌
                ***** use the standard OpenJDK project -> NOT include FEW small enhancements
            **** 's main goal
                ***** build NATIVE executables / -- designed to support -- Quarkus
            **** use cases
                ***** building NATIVE executables / -- target -- Linux containerized environments
                    ****** ❌== NOT support MacOs -- as -- target ❌
            **** if you want to build NATIVE executables DIRECTLY | bare metal Linux OR Windows -> see https://github.com/graalvm/mandrel/blob/default/README.md[Mandrel README]
...
* requirements
    ** generate NATIVE executables /
        ...
        *** target Linux /
            ...
            **** <<#container-runtime,use a builder image>>
* if you can NOT install GraalVM -> use a MULTI-stage Docker build / run Maven | Docker container / embeds GraalVM
    ** see <<#multistage-docker, here>>

GraalVM {graalvm-version} is required

1. if you have NOT already -> install GraalVM
    ** OPTIONS
        *** | <https://github.com/graalvm/mandrel/releases> OR <https://github.com/graalvm/graalvm-ce-builds/releases> & unpack it -- as -- ANY other JDK
        *** platform-specific installer tools
            **** https://sdkman.io/jdks#graalce[sdkman],
            **** https://github.com/graalvm/homebrew-tap[homebrew],
...
## | macOS (NOT supported by Mandrel), -- point to the -- `Home/`
export GRAALVM_HOME=$HOME/Development/graalvm/Contents/Home/
## | Windows, go through the Control Panel / set your environment
----
3. (OPTIONAL) set `JAVA_HOME` environment variable
+
[source,bash]
----
export JAVA_HOME=${GRAALVM_HOME}
----
4. (OPTIONAL) set GraalVM `bin` directory | `$PATH`
+
[source,bash]
----
...
[[graal-and-macos]]
.Issues using GraalVM -- via -- macOS
[NOTE]
====
* https://github.com/oracle/graal/issues/1724[GraalVM binaries -- are NOT notarized for -- macOS]
* `native-image`
+
[source,bash]
----
...
* recursively delete the `com.apple.quarantine` extended attribute | GraalVM install directory
+
[source,bash]
-----
...
* link:{quickstarts-tree-url}/getting-started[`getting-started/`]

== Producing a NATIVE executable

* TODO:
The native executable for our application will contain the application code, required libraries, Java APIs, and a reduced version of a VM.
...
* see xref:deploying-to-kubernetes.adoc[deployment | Kubernetes & OpenShift]
