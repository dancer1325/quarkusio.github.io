...
* icon:lock[title=Fixed at build time] Configuration property / fixed | build time
* REST of configuration properties
    ** are overridable | runtime

* properties / use NON-standard types
    ** can be referenced -- by -- expanding the property
        *** _Example:_ `kubernetes-readiness-probe`'s type == `Probe`
+
[source,properties]
----
